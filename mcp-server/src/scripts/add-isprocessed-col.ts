...
async function addIsProcessedColumn() {
  console.log('Adding column "isProcessed" to table "Vocabulary"...');
  ...
  if (res.status === 400) {
    ...
    if (text.includes('already exists')) {
      console.log('Column "isProcessed" already exists. Success.');
      return;
    }
  ...
  console.log('Column "isProcessed" added successfully.');
}

addIsProcessedColumn().catch(err => {
  console.error(err);
  process.exit(1);
});
