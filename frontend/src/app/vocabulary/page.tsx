...
export default function VocabularyPage() {
  ...
  if (loading) {
    return (
      <div className="flex min-h-screen items-center justify-center bg-gray-50 dark:bg-slate-950 text-gray-700 dark:text-gray-200">
        <div className="animate-pulse text-lg font-medium">Loading vocabulary lists...</div>
      </div>
    );
  }
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-8 bg-gray-50 dark:bg-slate-950 min-h-screen">
      <div className="mb-8">
        <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100 font-sans">Vocabulary Review</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          {words.length === 0 ? (
            <div className="bg-white dark:bg-slate-900 p-6 rounded-lg border border-gray-200 dark:border-slate-800 text-center shadow-sm">
              <p className="text-gray-500 dark:text-slate-400">All clear! No words currently pending review.</p>
            </div>
          ) : (
            words.map((word) => {
              ...
              return (
                <div key={word.id} className="bg-white dark:bg-slate-900 rounded-lg border border-gray-200 dark:border-slate-800 p-6 shadow-sm">
                  ...
                  <div className="text-xs text-gray-500 dark:text-slate-400 space-y-1 bg-gray-50 dark:bg-slate-950 p-3 rounded mb-4 border border-gray-100 dark:border-slate-800">
                    <div><strong>Daily Use</strong>: {word.fields.dailyUse}</div>
                    <div><strong>Professional Use</strong>: {word.fields.professionalUse}</div>
                  </div>
                  ...
                  {pendingReview ? (
                    <div className="bg-amber-50 dark:bg-amber-950/40 text-amber-800 dark:text-amber-300 text-xs p-3 rounded border border-amber-200 dark:border-amber-900/40 font-medium">
                      Sentence submitted: "{pendingReview.fields.userSentence}" (Pending offline AI correction)
                    </div>
                  ) : (
                    <div className="flex flex-col gap-2">
                      <input
                        type="text"
                        placeholder="Write a sentence in German using this word..."
                      ...
                      >
                        {submitting[word.id] ? "Submitting..." : "Submit Sentence"}
                      </button>
                    </div>
                  )}
                </div>
        ...
        <div className="space-y-6">
          <h2 className="text-lg font-semibold text-gray-900 dark:text-gray-100">Recent Corrections</h2>
          {reviews.filter((r) => r.fields.status !== "pending_correction").length === 0 ? (
            <div className="bg-white dark:bg-slate-900 p-6 rounded-lg border border-gray-200 dark:border-slate-800 text-center shadow-sm">
              <p className="text-gray-500 dark:text-slate-400">No corrections available yet.</p>
            </div>
          ) : (
            reviews
                  ...
                  <div key={review.id} className="bg-white dark:bg-slate-900 rounded-lg border border-gray-200 dark:border-slate-800 p-4 shadow-sm space-y-3">
                    <div className="flex items-center justify-between">
                      ...
                      <span className={`px-2 py-0.5 text-[10px] font-bold rounded ${
                        ...
                        {isCorrect ? "Corrected" : "Failed"}
                      </span>
                    </div>

                    <div className="text-xs space-y-2">
                      <div>
                        <div className="text-gray-500 dark:text-slate-400 font-medium">Your Sentence</div>
                        <div className="text-gray-900 dark:text-gray-200 line-through mt-0.5">"{review.fields.userSentence}"</div>
                      </div>
                      <div>
                        <div className="text-gray-500 dark:text-slate-400 font-medium">Corrected</div>
                      ...
                      {review.fields.correctionFeedback && (
                        <div>
                          <div className="text-gray-500 dark:text-slate-400 font-medium">Feedback</div>
                          <div className="text-gray-700 dark:text-slate-300 mt-0.5 italic">{review.fields.correctionFeedback}</div>
                        </div>
                      )}
                    </div>
                  </div>
