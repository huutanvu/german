...
- Articles (e.g. der, die, das, dem, den, des, ein, eine, einem, einen, einer) → return exactly the clean word form as clicked (e.g. if the input is 'dem', return exactly 'dem' as the resolved word, do not map it to 'der'). Do not add any parenthetical info, suffixes, or explanations (e.g. return "dem" and not "dem (dative)").
